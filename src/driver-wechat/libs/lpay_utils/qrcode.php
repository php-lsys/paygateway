...
//<!--
	//your code : window.__PayGateway={key:'callback',succ:function(url){},fail:function(status,msg){status&&alert(msg);}} //success callback
	(function(w){
        ...
        w.__PayGateway.getjson=getjson;
    })(window);
    //run ...
    (function(w){
    	var return_url='<?php echo $return_url?>',
    		check_url='<?php echo $check_url?>',
    		jsonobj=new w.__PayGateway.getjson(),
    		check_fn=function(){
    			jsonobj.set(check_url,function(data){
	       		   if(data.status){
		       			...
		       			w.__PayGateway.fail(0,data.data);
			       	 	setTimeout(check_fn,1500);
		       		}
	       	    },function(status,msg){
	       	    	if(w.__PayGateway.fail)w.__PayGateway.fail(status,msg);
	       			setTimeout(check_fn,1500);
	       	    },w.__PayGateway.key);
        	};
        w.__PayGateway=w.__PayGateway||{};
