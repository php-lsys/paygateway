...
class Tools{
	 ...
	 * 格式化参数格式化成url参数
	 */
	private static function _arrToQuery($arr)
	{
		$buff = "";
		foreach ($arr as $k => $v)
		{
	...
	public static function getToXml(array $arr,Config $config){
		$arr['appid']=$config->get("appid");
		$arr['mch_id']=$config->get("mchid");
		$arr['nonce_str']=self::_getNonceStr();
		$arr['sign']=self::_makeSign($arr, $config->get("key"));
		return self::toXml($arr);
	}
	...
	public static function parse($result,Config $config){
		...
		if (!isset($values['sign'])||$values['sign']!=$this->_makeSign($values, $config->get("key"))){
