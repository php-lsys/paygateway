...
(function(w){
	...
	w.__PayGateway.utils=w.__PayGateway.utils||{};
	w.__PayGateway.utils.ajax=ajax;
	w.__PayGateway.utils.on=bind_event;
	w.__PayGateway.utils.to_json=str_to_json;
	w.__PayGateway.utils.serialize_form=serialize_form;
})(window);
(function(w){
	var form=document.getElementById('_LSYS_PP_PAY_'),
		...
		fn_succ=function(){
			if(w.__PayGateway.succ)w.__PayGateway.succ(return_url);
			else window.location.href=return_url;
		},
		in_ajax=false,
		tips=function(status){
			if(w.__PayGateway.tips)w.__PayGateway.tips(status);
			if(in_ajax){
				in_ajax=status;				
				return false;
			}else{
	...
	utils.on(form,'submit',function(){
		...
		utils.ajax({
			...
			async : true,
			data :w.__PayGateway.utils.serialize_form(form),
			success : function (data) {
				tips(false);
				var _data=w.__PayGateway.utils.to_json(data);
