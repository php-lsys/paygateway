...
class Config{
	...
	public static function arr(array $config){
		...
		if (isset($config['currency_code'])&&isset($config['exchange_rate']))
			$self->setCurrency($config['currency_code'], $config['exchange_rate']);
		if (isset($config['mode']))$self->setMode($config['mode']);
		return $self;
	}
	 ...
	 * @return string[]
	 */
	public function setCurrency($currency_code,$exchange_rate){
